...
### 8.9 🔥 เวลาเพี้ยน +7 ชม. เฉพาะบน production — container ไม่ได้ตั้ง `TZ` (2026-07-25)

**อาการ:** แอดมินตั้งประกาศให้เริ่ม `25/07/2026 10:01` แต่ไม่โผล่บนเว็บสักที
ดูใน DB: `startsAt = 2026-07-25 10:01` ขณะที่ `now()` = `03:10` UTC → ระบบมองว่า "ยังไม่ถึงเวลา"
(จะโผล่จริงตอน 17:01 น. ไทย) · **บน dev ไม่เจอเลย**

**สาเหตุ:** `<input type="datetime-local">` ส่งค่า `"2026-07-25T10:01"` ที่ **ไม่มี offset**
→ `new Date(...)` ใน `parseEventDateInput()` ตีเป็น **เวลาท้องถิ่นของ process ที่รัน**
- dev = Windows `UTC+7` → ได้ `03:01Z` ✅ ถูก
- prod = container ที่ไม่ได้ตั้ง `TZ` → ค่าเริ่มต้น **UTC** → ได้ `10:01Z` ❌ เพี้ยน 7 ชม.

กระทบทุกที่ที่ใช้ helper ตัวนี้ (**ประกาศ + กิจกรรม**) และกระทบ **การแสดงผลวันที่ฝั่ง server** ด้วย
(`date-fns` `format`/`formatThaiDate` อิง TZ ของ process เหมือนกัน → วันที่ใกล้เที่ยงคืนจะโชว์ผิดวัน)

**✅ แก้:** ตั้ง `TZ: Asia/Bangkok` ที่ service `app` ใน `docker-compose.yml` — แก้ทั้ง parse และ format พร้อมกัน
ไม่ต้องแตะโค้ด และตรงกับสมมติฐานที่ทั้งแอปถูกเขียน+verify มา ("เวลาท้องถิ่นของ server = เวลาไทย")
...
> ✅ **ทดสอบแล้วว่า `node:22-alpine` รองรับ `TZ` ได้เลย ไม่ต้อง `apk add tzdata`**
> (ยิง `node -e` ในคอนเทนเนอร์จริง: `Intl...resolvedOptions().timeZone` = `Asia/Bangkok`,
> `new Date("2026-07-25T10:01").toISOString()` = `03:01Z` ✅)
> ⚠️ แต่ `date` ของ busybox ยัง**โชว์ UTC** เพราะอ่าน `/etc/localtime` ไม่ใช่ตัวแปร `TZ` → **อย่าใช้ `date` ในเชลล์ตัดสิน** ให้เช็คด้วย `node -e` เท่านั้น

**⚠️ `docker-compose.yml` บน VPS ไม่ได้อัปเดตตัวเองตอน deploy** — workflow แค่ `pull` image ใหม่
แก้ไฟล์นี้แล้วต้อง **`scp` ขึ้น VPS + `docker compose up -d app`** เอง ไม่งั้น push ไปก็ไม่มีผล

**📌 ข้อมูลที่บันทึกไปแล้วตอน TZ ยังผิด จะยังเพี้ยนอยู่** — ต้องเปิดแก้แล้วบันทึกใหม่ผ่าน `/admin`
(ฟอร์มจะโชว์เวลาที่ +7 ชม. ให้เห็นชัดว่าเพี้ยน) · แก้ที่ต้นทางแล้วของใหม่ถูกทันที

### 8.10 ฟีเจอร์ "จัดลำดับ" ที่มีอยู่แล้ว แต่ใช้จริงไม่ได้ — ทำเนียบบุคลากร (2026-07-26)

**อาการ:** เจ้าของแจ้งว่า "เพิ่ม ผอ. แล้วหน้าเว็บไม่เอาไว้บนสุด" ทั้งที่ `Staff.order` มีมาตั้งแต่ 4.4.5
และฟอร์มก็มีช่อง "ลำดับการแสดง" ให้กรอก

**สาเหตุ — สามชั้นซ้อนกัน ชั้นที่เจ็บสุดคือชั้นที่มองไม่เห็น:**

1. คนใหม่ได้ค่าแนะนำ `max+1` → **ต่อท้ายเสมอ** เพิ่ม ผอ. ทีหลังก็ไปอยู่ล่างสุด
2. หน้า public **จัดกลุ่มตาม `department` ก่อน** แล้วลำดับ*กลุ่ม*มาจาก "คนที่ `order` น้อยสุดในกลุ่ม"
   → **ตั้งลำดับกลุ่มตรง ๆ ไม่ได้เลย** ต้องไปไล่แก้เลขของคนในกลุ่มอื่นแทน ซึ่งเดาไม่ถูกว่าต้องแก้ใคร
3. ช่องกรอกเลขโชว์แค่เลขของคนคนเดียว — **มองไม่เห็นว่าเลขนั้นกำลังแข่งกับใคร** ต้องเปิดทีละหน้า
   (ตารางหลังบ้านยังแบ่งหน้าละ 10 อีก) แก้ทีนึงต้องเปิดหลายแท็บเทียบ

**บทเรียน:** ฟีเจอร์ "มีฟิลด์ให้กรอก" ≠ "จัดลำดับได้" ถ้า UI ไม่โชว์ **บริบทที่เลขนั้นถูกเทียบ**
ผู้ใช้จะสรุปว่า "ระบบเรียงไม่ได้" ไม่ใช่ "ฉันกรอกเลขผิด" — เพราะจากที่เขาเห็น มันเป็นความจริง

**✅ แก้:** เพิ่มตาราง `StaffDepartment` (ลำดับกลุ่ม) + ปุ่ม ▲▼ ที่ `/admin/staff` (หัวกลุ่ม + รายคน)
+ **ถอดช่องกรอกเลขทิ้ง** ให้ระบบคิดเลขเองทั้งหมด · หน้าหลังบ้านแสดงเป็นกลุ่มเหมือนหน้าเว็บจริงเป๊ะ

**⚠️ กับดักที่เจอตอนทำ (เผื่อมีโมดูลอื่นทำ reorder ตาม):**

- **หน้า list ที่ใช้จัดลำดับ ห้ามแบ่งหน้า** — คนท้ายหน้า 1 กด "เลื่อนลง" แล้วหายไปหน้า 2 ทันที
- **สองหน้าต้องเรียงด้วยโค้ดตัวเดียวกัน** (`lib/staff.ts`) ปุ่ม ▲▼ คิด "สุดขอบหรือยัง" จากลำดับที่เรนเดอร์
  ถ้า public กับ admin เรียงคนละแบบ กดแล้วผลไม่ตรงที่เห็น
- **เขียนเลขใหม่ทั้งชุด 0..n-1 ทุกครั้งที่สลับ** อย่าสลับแค่สองค่า — ข้อมูลเก่ามีเลขซ้ำ (`order` เท่ากันหมด)
  สลับสองค่าที่เท่ากันแล้วหน้าจอไม่ขยับ ดูเหมือนปุ่มเสีย
- **นับเฉพาะกลุ่มที่มีคนอยู่จริงตอนเลื่อนกลุ่ม** — เปลี่ยนชื่อฝ่ายแล้วแถวเก่าค้างเป็นกลุ่มร้าง ไม่ถูกเรนเดอร์
  ถ้านับรวมด้วย แอดมินจะกด ▲ หนึ่งครั้งแล้วเห็นว่า "ไม่ขยับ"
- **`department` ที่เป็น `''` ไม่ใช่ `NULL`** — `where: { department: null }` มองไม่เห็นแถวพวกนี้ กลายเป็นกลุ่มผี
  ที่ขยับไม่ได้ → migration ต้อง `UPDATE ... SET department = NULL WHERE btrim(department) = ''`

> 📌 **เขียน migration.sql เองได้ตอน DB ยังไม่ขึ้น** — วางไฟล์ใน `prisma/migrations/<timestamp>_<name>/`
> แล้วค่อย `prisma migrate dev` ทีหลัง ถ้า SQL ตรงกับ schema จริงจะขึ้น "Your database is now in sync"
> (ถ้าไม่ตรง Prisma จะฟ้อง drift ทันที — ใช้เป็นตัวตรวจได้เลย) · `prisma generate` ไม่ต้องใช้ DB

### 8.11 กรอบสัดส่วนตายตัว + คอนเทนต์แนวตั้ง = การ์ดขาวเปล่า (2026-07-26)

**อาการ:** เจ้าของแจ้งว่าการ์ดข่าว "เอา card มาครอบแล้วเหลือเนื้อที่ไม่สวย"
หน้า `/news` บน production ขึ้นเป็น**กล่องเทา/ขาวเปล่า**เกือบทั้งใบ ทั้งที่รูปโหลดสำเร็จ

**สาเหตุ:** `CoverImage` ตั้งใจ **ไม่ครอป** อยู่แล้ว (`object-contain` + พื้นหลังภาพเดิมเบลอ)
แต่รูปข่าวจริงเป็น **ประกาศ A4 แนวตั้ง 800×1132** ส่วนกรอบเป็น `aspect-[16/9]`
→ ย่อให้พอดี*ความสูง* เหลือภาพจริงกว้าง `170 × (800/1132) ≈ 120px` จากกรอบ `300px`
ที่เหลืออีก 60% เป็นพื้นหลังเบลอของกระดาษสีขาว → **มองยังไงก็เหมือนกล่องเปล่า**

> 🔍 **ยืนยันด้วยตัวเลข ไม่ใช่สายตา** — ดูจากสกรีนช็อตอย่างเดียวจะสรุปว่า "รูปโหลดไม่ขึ้น"
> ยิง JS อ่าน `naturalWidth/naturalHeight` ของ `<img>` จริงบนหน้า production ถึงเห็นว่า
> `complete: true, 800×1132` = โหลดสำเร็จแต่**สัดส่วนไม่เข้ากับกรอบ** คนละปัญหากันคนละทางแก้

**บทเรียน:** `object-contain` แก้แค่ "ไม่ครอป" ไม่ได้แก้ "ภาพเล็กจนอ่านไม่ออก"
ถ้าสัดส่วนคอนเทนต์จริงต่างจากกรอบมาก (แนวตั้ง A4 ใส่กรอบแนวนอน) **พื้นหลังเบลอยิ่งซ่อนปัญหา**
เพราะดูเผิน ๆ เหมือนกล่องมีสีสัน ไม่ได้ดูพังจนสังเกตเห็นตอนพัฒนา (ตอนเทสต์ใช้รูปแนวนอน)

**✅ แก้:** เพิ่มโหมด `natural` ใน `CoverImage` — ไม่มีกรอบ ภาพสูงตามสัดส่วนจริง ไม่ต้องมีพื้นหลังเบลอ
ใช้ที่กริด `/news` (คู่กับ `items-start`) และรูปปกหน้ารายละเอียด (คุมด้วย `max-h-[640px] w-auto`)

**⚠️ กับดักตอนทำ:**

- **`items-start` ที่ grid ขาดไม่ได้** — ค่าเริ่มต้น `stretch` จะยืดทุกใบในแถวให้สูงเท่าใบที่สูงสุด
  ภาพแนวนอนจะมีช่องว่างโหว่ใต้ข้อความ เสียจุดประสงค์ทั้งหมด
- **อย่าใส่ `bg-muted` ที่ wrapper ในโหมด natural** — พอผู้เรียกจำกัดความกว้างภาพ (`w-auto` + `max-h-*`)
  พื้นหลังจะโผล่เป็น**แถบเทาข้างภาพ** ให้ย้ายไปไว้ที่ `<img>` และใส่ `rounded` ที่ `<img>` ด้วย
  (ไม่งั้นมุมโค้งอยู่ที่ wrapper ที่กว้างกว่าภาพ = ภาพมุมเหลี่ยม)
- 📌 **โหมด natural มี layout shift ตอนโหลด** — DB เก็บแค่ URL ไม่มี width/height (แอดมินวางลิงก์
  โดเมนไหนก็ได้ ไม่ใช่แค่ Cloudinary) เบราว์เซอร์จองที่ล่วงหน้าไม่ได้ · ถ้าจะกำจัดจริงต้องเก็บขนาด
  ลง DB ตอนอัปโหลด (`CldUploadWidget` คืน `width`/`height` มาให้อยู่แล้ว) + ทางหนีทีไล่สำหรับลิงก์นอก

**⏳ ยังไม่แก้ (เจ้าของเลือกเอาไว้ก่อน 2026-07-26):** `components/public/news-section.tsx` บนหน้าแรก
ยังใช้กรอบ `aspect-[16/10]` / `aspect-[16/9]` อยู่ → **การ์ดข่าวหน้าแรกยังขึ้นเป็นกล่องขาวเปล่า**
เลย์เอาต์ตรงนั้นเป็นข่าวเด่นกินสองแถว + ใบเล็ก 2×2 ปล่อยภาพสูงอิสระแล้วกริดพัง ต้องเลือกระหว่าง
ครอป (`object-cover`) หรือรื้อเป็น 3 คอลัมน์เท่ากัน
