...
import { deleteMediaWork, toggleMediaWorkPublish } from "@/server/actions/media-work";

export function MediaWorkRowActions({
  id,
  title,
  status,
}: {
  ...
  function handleToggle() {
    startTransition(async () => {
      const result = await toggleMediaWorkPublish(id);
      if (!result.ok) {
  ...
  async function handleDelete() {
    const result = await deleteMediaWork(id);
    if (!result.ok) {
      toast.error(result.error);
      return;
    }
  ...
  return (
    <div className="flex items-center justify-end gap-1">
      ...
      <Button
        ...
        size="icon-sm"
        nativeButton={false}
        render={<Link href={`/admin/works/${id}/edit`} />}
        aria-label="แก้ไข"
        title="แก้ไข"
      >
        <Pencil />
