...
import { deleteAlbum, toggleAlbumPublish } from "@/server/actions/album";
...
export function AlbumRowActions({
  ...
  function handleToggle() {
    startTransition(async () => {
      const result = await toggleAlbumPublish(id);
      if (!result.ok) {
  ...
  return (
    <div className="flex items-center justify-end gap-1">
      ...
      <Button
        ...
        size="icon-sm"
        nativeButton={false}
        render={<Link href={`/admin/albums/${id}/edit`} />}
        aria-label="แก้ไข"
        title="แก้ไข"
      >
        <Pencil />
      ...
      <ConfirmDialog
        ...
        title="ลบอัลบั้มนี้?"
        description={`“${title}” และรูปทั้งหมดในอัลบั้มจะถูกลบถาวร กู้คืนไม่ได้`}
        confirmLabel="ลบอัลบั้ม"
